...
export default async function GetBirthdays(
  ...
  try {
    ...
    if (!number || Array.isArray(number) || isNaN(parseInt(number)) || parseInt(number) < 0) {
    ...
    return res.status(200).json({
      data: birthdaysData.length > itemsPerPage * parseInt(number)
        ? birthdaysData.length > itemsPerPage * (parseInt(number) + 1)
          ? birthdaysData.slice(itemsPerPage * parseInt(number), itemsPerPage * (parseInt(number) + 1))
          : birthdaysData.slice(itemsPerPage * parseInt(number), birthdaysData.length)
        : [] 
    })
  } catch(e:any) {
